...
\item Optimality: To argue that this approach computes the minimum number of status checks we use 'Greedy Algorithm Stays Ahead' argument. Consider, that our algorithm produces $ k $ status checks up to a certain process evaluation, and the set is denoted by $ S $. Also, there exists another set of status checks, $ S' $. We would like to show that $ S' $ too has at least $ k $ status checks up until the same process is evaluated. We prove this by induction:
\begin{enumerate}
